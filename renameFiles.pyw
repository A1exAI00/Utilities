...
layout = [[sg.T('Cкрипт копирует все файлы в выбранной папке в новую папку sorted под новым именем.\nФормат нового имени: 0003.png', size=(50,3))],
        ...
        [sg.I('', key='-DIR-',), sg.B('Выбрать папку', key='-CHOOSE-')],
        ...
        [sg.HSeparator()],
        [sg.T('Прогресс'), sg.ProgressBar(1000, orientation='h', size=(30, 20), key='-PROGRESS BAR-',style='xpnative')]]
...
while True:
    ...
    if event == '-CHOOSE-':
        folder_dir = str(sg.popup_get_folder('Выберите папку'))
        if folder_dir != None:
            window['-DIR-'].update(folder_dir)
